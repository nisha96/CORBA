...
public class SHEServer {
	public static void main(String args[]) 
	{
		try {
				...
				NameComponent path[] = ncRef.to_name("SHEServer");
				ncRef.rebind(path, ref);

				System.out.println("SHEServer is up and running");
					...
					byte[] buffer = new byte[1000];
		 			...
		 			while(true){
		 				DatagramPacket request = new DatagramPacket(buffer, buffer.length);
		  				...
		  				giveSHEResponse resp=new giveSHEResponse(aSocket,request,args);
...
class giveSHEResponse implements Runnable
{
	...
	String getData;
	byte[] m;
	public giveSHEResponse(DatagramSocket aSocket,DatagramPacket request,String[] args)
	{
